...
$artifact = Assert-Task001PathWithinRoot -RepositoryRoot $root -TargetPath $artifact
...
try {
    ...
    Assert-Task001TreeHasNoReparsePoints -Path $targetDirectory
    $buildFiles = @(Get-ChildItem -LiteralPath $targetDirectory -File -Recurse | Sort-Object FullName | ForEach-Object { $_.FullName })
    if ($buildFiles.Count -eq 0) { throw 'CIV001-BUILD-003: Build output tree contains no files.' }
    $diagnostics += [pscustomobject]@{ status = 'PASS'; code = 'CIV001-BUILD-000'; message = "$Target player built."; details = @{ log = $log; outputFiles = $buildFiles.Count } }
} catch {
...
Write-Task001CommandResult -Path $ResultPath -Command "Build/Build.ps1 -Target $Target" -Status $status -ExitCode $exitCode -StartedAt $started -Diagnostics $diagnostics -ArtifactPaths @($buildFiles + @($log)) -RepositoryRoot $root | Out-Null
